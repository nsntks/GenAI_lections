...
from .tool_directory_watcher import DirectoryWatcherTool
...
class LLMAgent:
    ...
    def _ask_llm_for_plan(self, query: str) -> List[Dict]:
        ...
        - **directory_watcher**: For monitoring folder changes (create, delete, modify files). 
          Commands:
          - start <path> - begin watching a folder
          - stop - stop watching
          - status - show current status
          - events [limit] - show last events
          - clear - clear events list
          Examples:
          - {{"action": "directory_watcher", "input": "start C:/temp"}}
          - {{"action": "directory_watcher", "input": "status"}}
          - {{"action": "directory_watcher", "input": "events 5"}}
